...
export default function sendChecks() {
  ...
  while (n = walk.nextNode()) {
    ...
    const object = {
      ...
      cmc: get(n, 'nodeValue'),
      vencimento: new Date(`${get(n, 'parentNode.previousElementSibling.textContent').split('/').reverse().join('-')} 00:00:00.0000`),
      valor: parseFloat(get(n, 'parentNode.previousElementSibling.previousElementSibling.textContent', '0').replace('.', '').replace(',', '.')),
      documento: get(n, 'parentNode.previousElementSibling.previousElementSibling.previousElementSibling.textContent'),
    };
